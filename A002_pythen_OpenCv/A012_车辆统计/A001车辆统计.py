...
while True:
    ...
    # 高斯模糊 去噪声
    GaussianBlur_img=cv2.GaussianBlur( cvtColor_img , (5,5),2)
    GaussianBlur_img=cv2.GaussianBlur( cvtColor_img , (5,5),2)
    cv2.imshow("GaussianBlur", GaussianBlur_img)
    # 腐蚀
    erode_img=cv2.erode( GaussianBlur_img, (3, 3), 2)
    cv2.imshow("erode", erode_img)
    # 膨胀
    dilate_iomg=cv2.dilate(erode_img, (3, 3),2)
    ...
    Canny_img=cv2.Canny( subtractor_img,10,100) 
